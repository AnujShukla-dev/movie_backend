...
//second task code code get request is allredy done in first task 

app.post("setname", async (req, res, next) => {
    try {
        if (req.body) {
            console.log("this is the dummy post req")
            res.json("this is dummy post req")
        }
        else {
            res.json("opss! you have missed the body ")
        }


    }
    catch (error) {
        error.message_key="some thing went wrong"
        next(error)
    }
})
